...
import com.nsw2022.todocatcheck.EditActivity
...
class TodoActivity : AppCompatActivity() {
    ...
    fun clickFinish(){
        val num:Int = todoItems[position].num
        val str:String=binding.etBsNote.text.toString()


        val db: SQLiteDatabase = openOrCreateDatabase("Todo", MODE_PRIVATE, null)
        db.execSQL("UPDATE todo SET note=? WHERE num=?", arrayOf(str,num.toString()))
        Toast.makeText(this, "수정이 완료되었습니다!", Toast.LENGTH_SHORT).show()

        todoItems[position].note=str
